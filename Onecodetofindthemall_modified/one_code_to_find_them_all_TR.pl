...
if($help==1){
  warn "#################\n";
  warn "Usage:\none_code_to_find_them_all.pl --rm <file> --ltr <file2> [--length <file3>] [--
   <file4> [--flanking <num>]] [--insert <num2>] [--strict] [--choice] [--unknown] [--dry-run]\n";
...
sub Wanted_length{
  ...
  if(/.*\.out$/  | ($file_ok==1)){
    ...
    while(<FILE>){
      ...
      next unless(($F[10] =~ /^DNA/) | ($F[10] =~/^SINE/) | ($F[10] =~ /^LINE/) | ($F[10] =~ /^LTR/) | (($F[10] =~/^Unknown|^Satellite|^rRNA|^Other|^Non-LTR_retrotransposon|^Simple_repeat/) & ($unknown == 1)));
...
sub Wanted{
  if(/.*\.out$/ | ($file_ok==1)){
	...
	$chrom_size{$F[4]}=$F[6]+$F[7];
      ...
      if(($F[10]=~/^Low_complexity/) | ($F[10] =~/^Simple_repeat/)){
	$useless_class = $F[10] ;
	$length = $F[6]-$F[5]+1;
	...
	next;
      ...
      if(($F[10] =~/^Unknown|^Satellite|^rRNA|^Other/) & ($unknown != 1)){
	$useless_class = $F[10] ;
	$length = $F[6]-$F[5]+1;
	$non_TE_length{$F[4]}{$useless_class}+=$length;
	$non_TE_count{$F[4]}{$useless_class}++;
	next;
      ...
      next unless(($F[10] =~ /^DNA/) | ($F[10] =~/^SINE/) | ($F[10] =~ /^LINE/) | ($F[10] =~ /^LTR/) | ($F[10] =~ /^RC/) | (($F[10] =~/^Unknown|^Satellite|^rRNA|^Other|^Non-LTR_retrotransposon/) & ($unknown == 1)));
	...
	print OUTSTAT "$contig\tFamily\tElement\tLength\tFragments\tCopies\tSolo_LTR\tTotal_Bp\tCover\n";
      }
	...
	foreach $elem (sort keys %{$h{$contig}{$family}}){
      ...
      if($unknown==1){
        push @families,"Type:rRNA";
        push @families,"Type:Satellite";
        push @families,"Type:Unknown";
        push @families,"Type:Other";
      }
      unless($dry==1){
